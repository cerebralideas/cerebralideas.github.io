...
A protected app (client or server) is simply an app that uses some type of access artifact to ensure a user’s identity and permissions prior to giving access to a resource. This “access artifact” can be a session cookie, an access token, or an assertion, and is shared between the entities of a system of applications and services.
...
To simply demonstrate our SDK integration, we will be using a simplified, non-production-ready, to-do app. This to-do app represents a Single Page Application with nothing more than React Router added to “the stack” for routing and redirection.

### Using this guide

This is a “hands on” guide. We are providing the web app and resource server for you. You can find the repo on Github. All you’ll need is your own ForgeRock Identity Cloud or Access Manager. If you don’t have access to either, and you are interested in ForgeRock’s Identity and Access Management platform, reach out to a representative today, and we’ll be happy to get you started.

#### Two ways of using this guide

1. Follow along by building the app yourself: continue with ensuring your meet the requirements below
2. Just read the React implementation details: skip to [Implementing the SDK](#implementing-the-sdk)
...
You will need a simple username and password login journey/tree for this guide.

1. Create a Page Node and connect it to the start of the journey/tree
2. Added the Username Collector and Password Collector nodes within the Page Node
...
5. Connect the `False` outcome of the decision nodeto the Failure node
...
Why two? It's conventional to have one OAuth client per app in the system. For this case, the OAuth client for the React app will provide our app with OAuth/OIDC tokens. The Node.js server will then validate the user's Access Token shared via the React app using its own OAuth client.
...
Create a simple test user within your ForgeRock server within the realm you will be using.
...
NOTE: The branch that will be used in this guide will be an overly-simplified version of the sample app you see in the `main` Git branch. There’s also a branch that represents the completion of this guide. If you get stuck, you can visit `blog/build-protected-app/complete` branch in Github. `<TODO: needs link>`

### Step 2. Create your security certificates

Apps that deal with authentication or authorization require HTTPS (secure protocol). HTTPS requires security (SSL/TLS) certificates. For local testing and development, it’s common to generate your own self-signed certificates.
...
In a *different* browser than the one you are using for administrating for ForgeRock server, visit the following URL: `https://react.example.com:8443`. A home page should be rendered explaining the purpose of the project. It should look like the below (it may be dark if you have a dark theme set in your OS):

![Image of web app in browser](https://share.getcloudapp.com/Apu98pkn)

If you encounter errors, here are a few tips:

- Directly visit `https://api.example.com:9443/health-check` and accept the security certificate, if required
- Ensure your `hosts` file has the correct aliases
- Check the terminal that has the `watch` command running for error output
- Ensure you are not logged into the ForgeRock server within the same browser as the sample app; logout from ForgeRock if you are
...
First, open up the `index.js` file, import the `Config` object from the JavaScript SDK and call the `set` method on this `Config` object:
...
The use of `set` should always be the first SDK method called and is frequently done at the application’s top-level file. To configure the SDK to communicate with the journeys/trees, OAuth clients, and realms of the appropriate ForgeRock server, pass a configuration object with the appropriate values.

Using the ForgeRock server that you setup earlier in the article, set those values in this configuration object. To reduce redundancy and risk of human error, we pull most of these values out of our `.env` variables which are mapped to constants within our `constants.js` file:
...
For the login page, the same pattern applies except it has less inline code:

`index.js` > `router.js` > `view/login.js` > `components/journey/form.js`

Navigate to the app’s login page within your browser. You should see a loading spinner and message that’s persistent. To ensure the correct form inputs are rendered, the initial data needs to be retrieved from the ForgeRock server. That will be the first task.
...
`FRAuth` is the first object used as it provides the necessary methods for authenticating a user against the Login **Journey**/**Tree**. We first use the `start` method of `FRAuth` as it returns data we need for rendering the form.
...
NOTE: We are passing an empty array as the second argument into `useEffect`. This instructs the `useEffect` to only run once after the component mounts.
...
Since a form that can’t submit anything isn't very usable, we'll now handle the submission of the user input values to ForgeRock. First, let’s edit the current form element, `<form className="cstm_form">`, and add an `onSubmit` handler with a simple, inline function.
...
The two important items to focus on is the `callback.getInputValue()` and the `callback.setInputValue()`. The `getInputValue` retrieves any existing value that may be provided by ForgeRock, and the `setInputValue` sets the user’s input on the callback while they are typing (i.e. `onChange`). Although we do not send this value to the `Form` component, we pass the `callback` to the components by “reference” (not by “value”). Any mutation of the `callback` object within the `Text` (or `Password`) component is also contained within the `step` object in `Form`.
...
Once you handle the success and error condition, return back to the browser, refresh the page and login with your test user created in the Setup section above. You should see your “Success!” alert message. Congratulations, you are now able to authenticate users!

### Step 4. Continuing to the OAuth 2.0 flow

At this point, the user is authenticated. The session has been created and a session cookie has been written to the browser. This would be session-based authentication, and is viable when your system (apps and services) can rely on cookies as the access artifact. However, [there are increasing limitations with the use of cookies](https://webkit.org/blog/10218/full-third-party-cookie-blocking-and-more/). In response to this, and other reasons, it's common to add an additional step to your authentication process: the “OAuth” or “OIDC flow”.

The goal of this flow is to attain a separate set of tokens, replacing the need for cookies as the shared access artifact. The two common tokens are the Access Token and the ID Token. We will focus on the access token in this guide. The specific flow that the SDK uses to acquire these tokens is the Authorization Code Flow with PKCE.

First, import the `TokenManager` object from the ForgeRock SDK.
...
Now, add a new `useEffect` hook to allow us to work with another asynchronous request. Unlike our first `useEffect`, this one to be dependent on the state of `isAuthenticated`. To do this, add `isAuthenticated` to the array passed in as the second argument.
...
Revisit the browser, clear out all cookies, storage and cache, and log in with you test user. If you landed on the home page and the logs in the console show tokens and user data, you have successfully used the access token for retrieving use data. Notice that the home page looks a bit different. This is due to the app “reacting” to the global state that we set just before the redirection.

### Step 6. Reacting to the presence of the access token

To ensure your app provides good UX, it’s important to have a recognizable, authenticated experience. Makes it clear to the user that they are logged in. This is done by persisting a portion of the global state to handle the user closing the tab or refreshing the browser. You can then easily verify their authentication status by checking the presence of stored tokens and immediately rendering the appropriate UI.

Adding the `TokenStorage.get` method to the `index.js` file will provide what we need to rehydrate the user's authentication status. First, import `TokenStorage` into the file and add it within the `initAndHydrate` function along with some of the user data the global state module adds to `localStorage`. Second, add these values to the `useGlobalStateMgmt` function call.
...
You can ensure the token is still valid with the use of `getCurrentUser` method earlier. This is optional, depending on your product requirements. If needed, you can protect routes with a token validation check before rendering portions of your application. This can prevent a potentially jarring experience of partial rendering that may be ejected due to an invalid token.
...
2. Find the the network call to the `/todos` endpoint (`https://api.example.com:9443/todos`)
...
Once all the above is complete, return to your browser, empty the cache, storage and cache, and reload the page. You should now be able to login with the test user, navigate to the todos page, add and edit some todos, and logout. Congratulations, you just built a ForgeRock protected app with React JS.
